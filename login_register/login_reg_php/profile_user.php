...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'increase') {
    ...
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("is", $amount, $user_email);
    ...
    if ($stmt->execute()) {
        ...
        $stmt->execute();
        $result = $stmt->get_result();
        $newCartera = $result->fetch_assoc()['cartera'];

        echo json_encode(["success" => true, "newCartera" => $newCartera]);
    } else {
        echo json_encode(["success" => false, "message" => "Failed to update cartera."]);
    }
    exit();
}
?>
...
<div class="container">
    ...
    <div class="right-side">
        <h3>Cartera</h3>
        <p id="carteraValue"><?php echo htmlspecialchars($cartera); ?></p>
        <button class="button" onclick="openIncreaseCarteraPopup()">Increase Cartera</button>
        <?php if ($is_fighter): ?>
...
<div id="increaseCarteraPopup" class="popup">
    <h3>Increase Cartera</h3>
    <form id="increaseCarteraForm" onsubmit="submitIncreaseCartera(event)">
        ...
        <button type="button" class="close-button" onclick="closePopup('increaseCarteraPopup')">Cancel</button>
    </form>
</div>
...
<script>
    ...
    function openIncreaseCarteraPopup() {
        document.getElementById('increaseCarteraPopup').style.display = 'block';
    }

    function submitIncreaseCartera(event) {
        event.preventDefault();
        var amount = document.getElementById('amount').value;
        var xhr = new XMLHttpRequest();
        xhr.open("POST", "profile_user.php", true);
        xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 200) {
                var response = JSON.parse(xhr.responseText);
                if (response.success) {
                    document.getElementById("carteraValue").textContent = response.newCartera;
                    closePopup('increaseCarteraPopup');
                } else {
                    alert("Error updating cartera: " + response.message);
                }
            }
        };
        xhr.send("action=increase&amount=" + amount);
    }
</script>
